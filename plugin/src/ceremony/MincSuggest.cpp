...
MincSuggestCtx MincBuildSuggestCtx(const std::string &presetKey, const std::string &pinPath) {
    ...
    std::vector<std::string> inp, view, render, viewOnly;
    JsonList(pr, "inputSpaces", inp);   if (inp.empty()) JsonList(j, "inputSpaces", inp);
    JsonList(pr, "viewSpaces", view);   if (view.empty()) JsonList(j, "viewSpaces", view);
    JsonList(pr, "viewOnly", viewOnly); if (viewOnly.empty()) JsonList(j, "viewOnly", viewOnly);
    JsonList(pr, "renderSpaces", render);
    if (render.empty()) JsonList(j, "renderSpaces", render);
    if (render.empty()) {                                        /* fallback: view+input dedup (:64) */
        std::map<std::string, bool> seen;
        for (auto &v : view) if (!seen.count(v)) { seen[v] = 1; render.push_back(v); }
        for (auto &v : inp) if (!seen.count(v)) { seen[v] = 1; render.push_back(v); }
    }
    {   /* viewOnly subtraction from render (:61-65) */
        std::map<std::string, bool> vo;
        for (auto &v : viewOnly) vo[v] = true;
        std::vector<std::string> r2;
        for (auto &v : render) if (!vo.count(v)) r2.push_back(v);
        render.swap(r2);
    }
    ...
    if (meta.valid && !meta.retired && !pinBase.empty() && pinBase != meta.config) {
        ...
        if (!ps.empty()) {
            ...
            keep(inp); keep(view); keep(render);
        }
    }
    for (auto &v : inp) { ctx.validInput[v] = true; ctx.validAll[v] = true; }
    for (auto &v : view) ctx.validAll[v] = true;
    for (auto &v : render) ctx.validAll[v] = true;
